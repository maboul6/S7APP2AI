...
class BayesClassifier(Classifier):
    ...
    def predict(self, data):
        ...
        # ---------------------------------------------------------------------
        weighted_probs = class_probabilities.T * self.aprioris
        risks = weighted_probs @ self.cost_matrix.T
...
class KNNClassifier(Classifier):
    ...
    def __init__(self, n_neighbors: int, use_kmeans: bool = False, n_representatives: int = 1, metric: str = "minkowski"):
        ...
        # ---------------------------------------------------------------------
        self.knn = sklearn.neighbors.KNeighborsClassifier(self.n_neighbors, metric=metric)
        # ---------------------------------------------------------------------
